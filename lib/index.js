...
exports.create = function(Arrow, server) {
	...
	return Connector.extend({
		...
		connect: function(callback) {
			...
			var connectionConfig = this.config.connection || this.config;
			...
			if (this.config.connectionPooling) {
				...
				pg.connect(connectionConfig, function connectPool(error, client, done) {
					if (error) {
						callback(error);
					} else {
						done();
				...
				this.connection = new pg.Client(connectionConfig);

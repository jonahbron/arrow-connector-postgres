	...
	return Connector.extend({
		...
		getConnection: function getConnection(callback) {
			if (this.connection) {
				...
				pg.connect(config, allocateConnection.bind(null, this.config, callback));
			}
		},
...
function allocateConnection(config, callback, error, connection, done) {
	if (error) {
		callback(error);
		...
		config.onAllocateConnection(
			...
			function afterConnectionAllocationCallback() {
				callback(null, connection, deallocateConnection.bind(null, config, connection, done));
			}
		);
	} else {
		callback(null, connection, deallocateConnection.bind(null, config, connection, done));
...
function deallocateConnection(config, connection, done) {
	if (config.onDeallocateConnection) {
		config.onDeallocateConnection(connection, done);
	} else {
		done();
	}
}

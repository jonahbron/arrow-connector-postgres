	...
	return Connector.extend({
			...
			this._query(query, callback, function(results){
				...
				results.rows.forEach(function resultCallback(result) {
					...
					if (result.constraint_type === 'PRIMARY KEY' ||
						(!schema.primary_keys[result.table_name] &&
		...
		query: function (Model, options, callback) {
			...
			if (sel && Object.keys(sel).length > 0) {
				keys = (keys ? keys + ', ' : '') + _.keys(_.omit(sel, primaryKeyColumn)).join(', ');
			}
			else if (unsel && Object.keys(unsel).length > 0) {
				keys = (keys ? keys + ', ' : '') + _.keys(_.omit(_.omit(this.getTableSchema(Model), primaryKeyColumn), _.keys(unsel))).join(', ');
			}
			else {
				keys = '*';
			}
		...
		whereQuery: function(where, values) {
			var valuePosition = function(value) {
				values.push(value);
				return '$' + String(parameterPosition++);
			};
			...
			for (key in where) {
				if (where.hasOwnProperty(key) && where[key] !== undefined) {
					...
					if (where[key] && where[key].$like) {
						...
						where[key].$sql += ' '; // Pad to fix number matching
						for (var i = 0; where[key].values && i < where[key].values.length; i++) {
							where[key].$sql = where[key].$sql.replace(new RegExp('\\$' + (i + 1) + '([^\d])', 'g'), '$' + valuePosition(where[key].values[i]) + '$1');
						}

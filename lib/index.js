...
exports.create = function (Arrow, server) {
	...
	return Connector.extend({
        ...
        _groomPayload: function(Model, payload, allowNull) {
            ...
            if (allowNull) {
                // translate all undefined values to null to support clearing postgres fields
                payload = _.transform(payload, function(result, value, key) {
                    result[key] = (value === undefined) ? null : value;
                }, {});
            } else {
                // remove all columns with undefined payload to support postgres defaults
		...
		create: function (Model, values, callback) {
            ...
            payload = this._groomPayload(Model, payload, false);
		...
		save: function (Model, instance, callback) {
			...
			payload = this._groomPayload(Model, payload, true);
			...
			this._query(query, values, request, response, callback, function(result) {
				if (result.rows && result.rows.length > 0 && result.rowCount) {
					callback(null, instance);
				}
				else {
					callback();
        ...
        updateAll: function (Model, payload, options, callback) {
		    var whereQuery = '';
            var values = [];
            ...
            var query = 'UPDATE ' + table + ' SET ' + placeholders.join(',') + whereQuery;
			...
			for (key in where) {
				if (where.hasOwnProperty(key) && where[key] !== undefined) {
					...
					if (where[key] && where[key].$like) {
                        ...
                        whereQuery += ' <> ' + valuePosition(where[key].$ne);
		...
		fetchColumns: function fetchColumns(table, payload) {
            ...
            var schema = this.schema.objects[table];
            if (schema) {
                var payLoadKeys = Object.keys(payload);
                var fieldsToRemove = _.difference(payLoadKeys, Object.keys(schema));
                return _.without(payLoadKeys, fieldsToRemove);
            }
		...
		getReadReplicaPool: function getReadReplicaPool() {
			...
			// Rotate and carry pools to change stable sorting of identical pools
			this.readReplicaConnectionPools.push(this.readReplicaConnectionPools.shift());
			...
			this.getConnection(function (error, client, done) {
				...
				if (config.logLevel && config.logLevel > config.logLevels.NONE) {
                    ...
                    if (config.logLevel === config.logLevels.DEBUG) {
                        logger.info(localISOTime + ' - connection id: ' + client.id + ' - ' + query + ' [' + data + '] effective_user: (' + client.effective_user_id + ')');
                    } else {
					...
					if (error) {
                        ...
                        if (config.logLevel && config.logLevel > config.logLevels.NONE) {
                            ...
                            if (config.logLevel === config.logLevels.TRACE) {
                                logger.info(localISOTime + ' - connection id: '+clientId+' - '+query, data, 'result count:', results.rows.length);

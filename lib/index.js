				...
				if (where.hasOwnProperty(key) && where[key] !== undefined) {
					...
					if (where[key] && where[key].$like) {
                        ...
                        if (where[key].length > 0) {
                            ...
                            where[key].forEach(function(entry) {
                                _.forOwn(entry, function (entryValue, entryKey) {
                                    whereQuery += ' ' + entryKey;
                                    whereQuery += ' <> ';
                                    whereQuery += valuePosition(entryValue);
                                });
                            });
                        }
			...
			this.getConnection(function (error, client, done) {
				...
				client.query(query, data, function (error, results) {
					...
					if (error) {
                        ...
                        if (startTime) {
                            logger.info(query, 'result count:', results.rows.length, data);
                            logger.info('query response time: '+(Date.now()-startTime)+'ms');
                        }
